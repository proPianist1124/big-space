				...
				if(await db.get("postNumber") == null){
					...
					for (let i = 1; i < parseInt(await db.get("postNumber")); i++) {
						...
						(async () => {
							...
							let structure = `<div class = "postcard"><h2><span style = "color:var(--primary)">${postString.title}</span>${menu}</h2><span>${postString.content}</span><br><br>${postString.image}<br><span style = "color:var(--secondary)">${postString.date}, ${postString.topic}</span><br><a href = "/@${user.name}" class = "default"><img src = "${user.profile}" class = "pfp"/>&nbsp;&nbsp;@${user.name}&nbsp;&nbsp;${badge}</a><br><br></div> ${posts}`;
