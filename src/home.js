	...
	app.get("/", function(req, res) {
		...
		(async () => {
			...
			if (req.cookies.name == "" || req.cookies.name == null || req.cookies.name == undefined || user == null) {
				...
				if (location != await db.get(`${user}_address`) || await db.get(`${user}_address`) == null || location == process.env['banned']) { // layer to check if the user's address is the same address as the one when the account was first created or if the address is not valid (meaning the cookie is fake)
